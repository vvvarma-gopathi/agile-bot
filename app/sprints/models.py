...
class SprintItems(Base):
    ...
    task_id=Column(UUID(as_uuid=True),ForeignKey('tasks.id'),nullable=False,default=uuid4)
    added_at=Column(DateTime(timezone=True),server_default=func.now(),nullable=False)

class Sprints(Base):
    __tablename__='sprints'
    id=Column(UUID(as_uuid=True),primary_key=True,default=uuid4)
    project_id=Column(UUID(as_uuid=True),ForeignKey('projects.id'),default=uuid4,nullable=False)
